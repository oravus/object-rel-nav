...
class MatchLightGlue():
    def __init__(self, resize_w=320, resize_h=240, device='cuda', cfg={}):
        ...
        self.lexor = SuperPoint(max_num_keypoints=2048, detection_threshold=0.0).eval().to(self.device)
        self.lmatcher = LightGlue(features='superpoint', depth_confidence=-1, width_confidence=-1, mp=True).eval().to(self.device)
    ...
    def matchPair_imgWithMask(self, imSrc, imTgt, nodesSrc, nodesTgt, visualize=False, matcher=None, extractor=None, ftSrc=None, ftTgt=None, lmatches=None):
    ...
    def matchPair_imgWithMask_multi(self, qryImg, refImgList, qryNodes, refNodesList, visualize=False, ftTgtList=None):
        ...
        if use_batching:
            ...
            for b in range(num_batches):
                ...
                with torch.no_grad():
                    ...
                    ftSrcBatch = {
                        key: torch.cat([ftSrc[key] for _ in range(len(refImgBatch))], dim=0).to(self.device).half()
        ...
        for i in range(len(refImgList)):
            ...
            if lmatchesBatch is not None:
                lmatches = {key:lmatchesBatch[key][i] for key in lmatchesBatch.keys() if key in ['matches', 'scores']}

            results = self.matchPair_imgWithMask(
                qryImg, refImg, qryNodes, refNodes, visualize, ftSrc=ftSrc, ftTgt=ftTgt, lmatches=lmatches)
